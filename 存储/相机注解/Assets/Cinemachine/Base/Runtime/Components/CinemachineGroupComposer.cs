...
namespace Cinemachine
{
    ...
    /// is framed properly.
    /// 相对于composer是添加了targetGroup功能，TargetGroup功能和Follow的一致，会自适应距离和fov来讲targetgroup的bounds完全在视野内。
    /// 由于需要改变fov所以aim部分需要再次计算，所以流程是先使用composer计算，然后进行group bouns适配，然后再次使用composer计算
    /// </summary>
    ...
    {
        /// <summary>How much of the screen to fill with the bounding box of the targets.
        /// Bounds占有屏幕的比例
        /// </summary>
        [Space]
        ...
        public override void MutateCameraState(ref CameraState curState, float deltaTime)
        {
            ...
            //如果偏差小于0，则说明相机和需要看向的点重合，则无需进行相机的旋转了
            ...
            //构建一个矩阵空间-以相机的forward和指定Up组成的旋转为角度，以bounds.center - (fwd * bounds.extents.magnitude)
            //为原点的矩阵。
            ...
            //-求出我们看向的bounds的多少，我们最终看向是的targetgroup中的点，但是我们要确保
            //targetgroup组成的bounds我们要看到，看到多少有targetHeight决定。怎么才能确保targetgroup的bounds
            //能够看到我们可以使用移动相机和调整fov来实现。
            m_prevTargetHeight = targetHeight;
            //调整相机--有一点不明白的是，要实现targetheight在视图内指定位置，通过移动相机位置和zoom都可以实现
            //所以不明白调节模式为DollyThenZoom时，已经通过位移实现了为什么还要进行Zoom
            // Move the camera
            if (!curState.Lens.Orthographic && m_AdjustmentMode != AdjustmentMode.ZoomOnly)
            {
                ...
                //在当前的FOV的情况下，屏幕内显示指定的targetHeight，需要将摄像机放在多远的地方
                ...
                // Apply
                //计算矫正位置=目标位置-计算后摄像机距离目标的距离-当前相机的位置
            ...
            if (curState.Lens.Orthographic || m_AdjustmentMode != AdjustmentMode.DollyOnly)
            {
                ...
                //如果是正交相机，设置正交相机的OrthographicSize
