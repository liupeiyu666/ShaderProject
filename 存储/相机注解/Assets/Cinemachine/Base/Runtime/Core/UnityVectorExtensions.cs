...
namespace Cinemachine.Utility
{
    ...
    public static class UnityVectorExtensions
    {
        ...
        /// 求一个向量在某个平面上投影。planeNormal表示这个平面的投影
        /// </summary>
        ...
        /// <summary>计算向量from和to的夹角，refnormal方向为正方向，反之为负夹角</summary>
    ...
    public static class UnityQuaternionExtensions
    {
        ...
        /// <summary>
        /// 获取旋转角orient和lookAtDir在一个以worldup为Up的坐标系中，两者相对于worldup和worldup坐标系的right的角度
            ...
            this Quaternion orient, Vector3 lookAtDir, Vector3 worldUp)
        {
            ...
            // Work in local space
            //-将lookAtDir和worldUp转换到orient旋转的坐标空间
            ...
            float angleH = 0;
            {
                //-计算在up垂直的平面（xz平面）的投影,这个up都是state.ReferenceUp
                //targetDirH就是lookAtDir和state坐标系xz平面的夹角
                Vector3 targetDirH = lookAtDir.ProjectOntoPlane(up);
                if (!targetDirH.AlmostZero())
                {
                    //-Vector3.forward在相机坐标系中就是相机的正前方，这里相当于相机的正前方和平面的夹角  
