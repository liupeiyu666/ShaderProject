...
namespace Cinemachine
{
    ...
    /// 使用相机列表来控制相机的切换，而相机的切换方式是时间按照Hold的时长进行计算，
    /// 注意Blend的时长并不影响相机的切换时间，比如相机B的EaseIn的时长是2秒，Hold时长是1米，则切换到
    /// 相机B那一刻开始计时，到了1秒后进入下一个相机，此时融合只进行到1秒，剩下的时间不会继续进行了
    /// </summary>
    ...
    {
        /// <summary>Default object for the camera children to look at (the aim target), if not specified in a child rig.  May be empty
        /// 子相机默认的看向的目标，如果子相机有目标则忽略
        /// </summary>
        ...
        /// <summary>Force all child cameras to be enabled.  This is useful if animating them in Timeline, but consumes extra resources.
        /// 是否激活所有相机
        /// </summary>
        ...
        /// <summary>This represents a single entry in the instrunction list of the BlendListCamera.
        /// 每个相机的信息，持续多久，以及切换的时间设置
        /// </summary>
        [Serializable]
        ...
        public override void UpdateCameraState(Vector3 worldUp, float deltaTime)
        {
            ...
            if (m_ChildCameras != null)
            {
                for (int i = 0; i < m_ChildCameras.Length; ++i)
                {
                    ...
                    if (vcam != null)
                    {
                        //是否激活相机===如果设置了激活所有相机就激活所有相机，否则只激活当前使用的相机
            ...
            if (best != null)
            {
                ...
                //是否发生了切换，当前相机和上一个相机不一致则认为开始切换需要进行融合
            ...
            //如果Blend没有完成，则使用Blend的状态，如果没有Blend或者Blend已经结束，则使用当前激活的相机
            if (mActiveBlend != null)
            {
        ...
        /// <summary>
        /// 设置当前使用的相机混合设置的下标--计算Hold时长，混合时长
        /// </summary>
        ...
        /// <param name="camB"></param>
        /// <param name="blendCurve"></param>
        /// <param name="duration"></param>
        /// <param name="activeBlend"></param>
