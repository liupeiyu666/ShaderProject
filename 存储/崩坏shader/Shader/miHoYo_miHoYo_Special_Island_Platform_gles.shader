...
void main ()
{
  ...
  lowp vec3 tmpvar_5;
  tmpvar_5 = texture2D (_ShadowTex, xlv_TEXCOORD1).xyz;
  shadowColor_1 = tmpvar_5;
  outColor_3.xyz = ((baseTexColor_2.xyz * 2.0) * (shadowColor_1 * _LightMapIntensity));
  if ((_EmissionToggle > 0.5)) {
    outColor_3.xyz = (outColor_3.xyz * mix (1.0, _EmissionScaler, baseTexColor_2.w));
  };
  if ((_EmissionToggle > 0.5)) {
    outColor_3.w = mix (_BloomFactor, _EmissionBloomFactor, baseTexColor_2.w);
  } else {
