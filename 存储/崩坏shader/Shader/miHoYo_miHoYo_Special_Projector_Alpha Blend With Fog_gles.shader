...
void main ()
{
  ...
  tmpvar_1 = (unity_Projector * _glesVertex);
  tmpvar_2 = ((tmpvar_1.xy * _FalloffTex_ST.xy) + _FalloffTex_ST.zw);
...
void main ()
{
  mediump vec4 falloff_1;
  mediump vec4 res_2;
  lowp vec4 tmpvar_3;
  tmpvar_3 = texture2DProj (_Cookie, xlv_TEXCOORD0);
  res_2 = tmpvar_3;
  lowp vec4 tmpvar_4;
  tmpvar_4 = texture2D (_FalloffTex, xlv_TEXCOORD1);
  falloff_1 = tmpvar_4;
  res_2.w = (((
    ((1.0 - res_2.x) * _Opaqueness)
   * _Falloff) * _MainColor.w) * falloff_1.x);
  mediump float x_5;
  x_5 = (res_2.w - 0.01);
  if ((x_5 < 0.0)) {
    discard;
  };
  res_2.xyz = mix (_MainColor.xyz, xlv_COLOR1.xyz, xlv_COLOR1.www);
...
void main ()
{
  ...
  res_2.xyz = (xlv_COLOR1.xyz + (_MainColor.xyz * clamp (xlv_COLOR1.w, 0.0, 1.0)));
  res_2.w = clamp (res_2.w, 0.0, 1.0);
  gl_FragData[0] = res_2;
}
