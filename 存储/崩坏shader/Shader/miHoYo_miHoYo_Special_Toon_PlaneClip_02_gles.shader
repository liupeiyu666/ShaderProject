...
void main ()
{
  ...
  tmpvar_13 = pow (max (dot (tmpvar_9, 
    normalize((tmpvar_7.xyz + normalize((_WorldSpaceCameraPos - tmpvar_12.xyz))))
  ), 0.0), _Shininess);
  s_1 = tmpvar_13;
  if ((d_2 <= 0.5)) {
    s_1 = 0.025;
  ...
  tmpvar_16 = dot (tmpvar_12, _ClipPlane);
  if ((tmpvar_16 < 0.0)) {
    tmpvar_3.w = 0.0;
  };
...
void main ()
{
  ...
  tmpvar_11 = dot ((unity_ObjectToWorld * _glesVertex), _ClipPlane);
  if ((tmpvar_11 < 0.0)) {
    tmpvar_6.w = 0.0;
  };
  gl_Position = tmpvar_5;
