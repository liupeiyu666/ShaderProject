...
void main()
{
    ...
    u_xlat0.x = u_xlat16_0.x * u_xlat10;
    ...
    u_xlat5.xy = vec2(u_xlat16_5) * vec2(_RimPow, _RimPow2);
    u_xlat5.xy = exp2(u_xlat5.xy);
    u_xlat15 = min(_RimSterngth, 1.0);
    u_xlat2.xw = vec2(u_xlat15) * u_xlat5.xy;
#ifdef UNITY_ADRENO_ES3
    u_xlat2.xw = min(max(u_xlat2.xw, 0.0), 1.0);
#else
    u_xlat2.xw = clamp(u_xlat2.xw, 0.0, 1.0);
#endif
    u_xlat5.x = vs_TEXCOORD3.y + (-_BottomPos);
    u_xlat5.x = (-u_xlat5.x) + vs_TEXCOORD2.w;
    u_xlat5.x = u_xlat5.x / _BottonWidth;
#ifdef UNITY_ADRENO_ES3
    u_xlat5.x = min(max(u_xlat5.x, 0.0), 1.0);
#else
    u_xlat5.x = clamp(u_xlat5.x, 0.0, 1.0);
#endif
    u_xlat4.xyz = _Colortop.xyz + (-_Colorbottom.xyz);
    u_xlat5.xyz = u_xlat5.xxx * u_xlat4.xyz + _Colorbottom.xyz;
    u_xlat4.xyz = (-u_xlat5.xyz) + _RimColor.xyz;
    u_xlat5.xyz = u_xlat2.xxx * u_xlat4.xyz + u_xlat5.xyz;
    u_xlat4.xyz = u_xlat10_1.xyz * _StarColor.xyz + (-u_xlat5.xyz);
    u_xlat2.xyz = u_xlat0.xxx * u_xlat4.xyz + u_xlat5.xyz;
    SV_Target0 = u_xlat2;
    return;
}
