...
#ifdef UNITY_ADRENO_ES3
    u_xlatb0 = !!(vec4(0.0, 0.0, 0.0, 0.0)!=vec4(_ReverseHue));
#else
    u_xlatb0 = vec4(0.0, 0.0, 0.0, 0.0)!=vec4(_ReverseHue);
#endif
    u_xlat16_9.x = (-u_xlat16_3.x) + _HueOffset;
    u_xlat16_1.y = fract(u_xlat16_9.x);
    u_xlat16_1.xzw = u_xlat16_3.xyz + vec3(_HueOffset, _SaturateOffset, _ValueOffset);
    u_xlat16_1.zw = u_xlat16_1.zw;
#ifdef UNITY_ADRENO_ES3
    u_xlat16_1.zw = min(max(u_xlat16_1.zw, 0.0), 1.0);
#else
    u_xlat16_1.zw = clamp(u_xlat16_1.zw, 0.0, 1.0);
#endif
    u_xlat16_1.x = fract(u_xlat16_1.x);
    u_xlat16_9.xyz = (bool(u_xlatb0)) ? u_xlat16_1.yzw : u_xlat16_1.xzw;
