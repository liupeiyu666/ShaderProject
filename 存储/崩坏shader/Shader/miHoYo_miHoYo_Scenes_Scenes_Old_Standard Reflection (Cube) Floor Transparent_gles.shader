...
void main ()
{
  ...
  tmpvar_2 = (unity_MatrixVP * (unity_ObjectToWorld * tmpvar_10));
  tmpvar_3 = ((_glesMultiTexCoord0.xy * _MainTex_ST.xy) + _MainTex_ST.zw);
  tmpvar_4 = ((_glesMultiTexCoord0.xy * _ReflectionTex_ST.xy) + _ReflectionTex_ST.zw);
...
void main ()
{
  mediump vec3 reflColor_1;
  mediump vec3 I_2;
  mediump vec3 normalFromMap_3;
  ...
  diffColor_4 = ((diffColor_4 * (
    (((2.0 * ShadowColor_5.xyz) * ShadowColor_5.w) + 1.0)
   - ShadowColor_5.w)) * _LightMapIntensity);
  lowp vec3 tmpvar_11;
  tmpvar_11 = ((texture2D (_NormalMap, xlv_TEXCOORD2).xyz * 2.0) - 1.0);
  normalFromMap_3 = tmpvar_11;
  mediump vec3 tmpvar_12;
  tmpvar_12 = mix (xlv_TEXCOORD5, normalFromMap_3, vec3(_NormalMapStrength));
  highp vec3 tmpvar_13;
  tmpvar_13 = normalize((_WorldSpaceCameraPos - xlv_TEXCOORD4));
  I_2 = tmpvar_13;
  mediump float power_14;
  power_14 = _FresnelParams.x;
  mediump float scale_15;
  scale_15 = _FresnelParams.y;
  mediump float bias_16;
  bias_16 = _FresnelParams.z;
   ...
   * tmpvar_12)));
  lowp vec3 tmpvar_19;
  tmpvar_19 = textureCube (_ReflectionTex, tmpvar_17).xyz;
  reflColor_1 = tmpvar_19;
  reflColor_1 = (reflColor_1 * _ReflectionEmissionAdjust);
  outColor_7.xyz = (mix (diffColor_4, reflColor_1, vec3((
    (bias_16 + max (0.0, (pow (
      clamp ((1.0 - dot (I_2, xlv_TEXCOORD5)), 0.001, 1.0)
    , power_14) * scale_15)))
   * baseTexColor_6.w))) + ((
    ((max (0.0, pow (
      max (dot (tmpvar_12, normalize((
        -(normalize(_LightDir))
       + I_2))), 0.001)
    , _Shininess)) * _SpecularIntensity) * baseTexColor_6.w)
   * _SpeculaColor) * baseTexColor_6.w));
  outColor_7.w = xlv_TEXCOORD6;
  outColor_7.xyz = mix (outColor_7.xyz, xlv_COLOR1.xyz, xlv_COLOR1.www);
  outColor_7.w = clamp (xlv_TEXCOORD6, 0.0, 1.0);
...
void main ()
{
  ...
  tmpvar_3 = ((_glesMultiTexCoord0.xy * _ReflectionTex_ST.xy) + _ReflectionTex_ST.zw);
  tmpvar_4 = ((_glesMultiTexCoord0.xy * _NormalMap_ST.xy) + _NormalMap_ST.zw);
  ...
  highp vec3 tmpvar_17;
  tmpvar_17 = (tmpvar_12.xyz - _WorldSpaceCameraPos);
  ...
  tmpvar_31 = clamp (((tmpvar_12.y * _HeigtFogColParams.x) + _HeigtFogColParams.y), 0.0, 1.0);
  tmpvar_30 = tmpvar_31;
...
void main ()
{
   ...
   * _SpeculaColor) * baseTexColor_6.w));
  outColor_7.w = xlv_TEXCOORD6;
  outColor_7.xyz = (xlv_COLOR1.xyz + (outColor_7.xyz * clamp (xlv_COLOR1.w, 0.0, 1.0)));
  outColor_7.w = clamp (xlv_TEXCOORD6, 0.0, 1.0);
  gl_FragData[0] = outColor_7;
}
...
void main ()
{
  ...
  mediump float tmpvar_30;
  highp float tmpvar_31;
  tmpvar_31 = clamp (((tmpvar_12.y * _HeigtFogColParams2.x) + _HeigtFogColParams2.y), 0.0, 1.0);
  tmpvar_30 = tmpvar_31;
  h_24 = (tmpvar_30 * (2.0 - tmpvar_30));
  highp vec3 tmpvar_32;
  ...
  tmpvar_65 = clamp (((tmpvar_12.y * _HeigtFogColParams3.x) + _HeigtFogColParams3.y), 0.0, 1.0);
  tmpvar_64 = tmpvar_65;
