...
void main ()
{
  ...
  mediump vec3 I_12;
  I_12 = tmpvar_10;
  tmpvar_11 = (_ReflectionFresnelFactors.z + max (0.0, (
    pow (clamp ((1.0 - I_12.y), 0.001, 1.0), _ReflectionFresnelFactors.x)
   * _ReflectionFresnelFactors.y)));
  tmpvar_6.x = tmpvar_11;
  highp vec3 tmpvar_13;
  tmpvar_13 = normalize(tmpvar_8);
  mediump float tmpvar_14;
  mediump vec3 I_15;
  I_15 = tmpvar_13;
  tmpvar_14 = (_CausticFresnelFactors.z + max (0.0, (
    pow (clamp ((1.0 - I_15.y), 0.001, 1.0), _CausticFresnelFactors.x)
   * _CausticFresnelFactors.y)));
  tmpvar_6.w = tmpvar_14;
  tmpvar_6.y = pow (sqrt(dot (tmpvar_8, tmpvar_8)), 0.5);
...
void main ()
{
  ...
  tmpvar_6 = clamp ((xlv_TEXCOORD4.x * xlv_TEXCOORD4.z), 0.0, 1.0);
  mediump vec4 tmpvar_7;
  tmpvar_7 = mix (_RefractionColor, _WaterColor, vec4(tmpvar_6));
  outcolor_1.w = tmpvar_7.w;
  lowp vec4 tmpvar_8;
  highp vec2 P_9;
  P_9 = (xlv_TEXCOORD2 - (texoffset_2 * 5.0));
  tmpvar_8 = texture2D (_CausticMap, P_9);
  outcolor_1.xyz = (tmpvar_7.xyz + (tmpvar_8.xyz * xlv_TEXCOORD4.w));
  outcolor_1.xyz = mix (outcolor_1.xyz, xlv_COLOR1.xyz, xlv_COLOR1.www);
  outcolor_1.w = clamp (tmpvar_7.w, 0.0, 1.0);
...
void main ()
{
  ...
  tmpvar_5.y = pow (sqrt(dot (tmpvar_8, tmpvar_8)), 0.5);
  mediump vec4 tmpvar_16;
