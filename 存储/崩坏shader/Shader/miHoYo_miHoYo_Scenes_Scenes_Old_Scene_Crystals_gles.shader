...
//
///////////////////////////////////////////
Shader "miHoYo/miHoYo_Scenes/Scenes_Old/Scene_Crystals" {
Properties {
...
{
  mediump vec3 I_1;
  mediump vec4 Emission_2;
  mediump vec3 reflection_3;
  mediump vec3 R_4;
  mediump vec4 tex_C_Color_5;
  lowp vec4 tmpvar_6;
  tmpvar_6 = texture2D (_MainTex, xlv_TEXCOORD0);
  tex_C_Color_5 = tmpvar_6;
  highp vec3 tmpvar_7;
  tmpvar_7 = normalize((xlv_TEXCOORD1.xyz - _WorldSpaceCameraPos));
  highp vec3 tmpvar_8;
  tmpvar_8 = (tmpvar_7 - (2.0 * (
    dot (xlv_TEXCOORD2, tmpvar_7)
   * xlv_TEXCOORD2)));
  R_4 = tmpvar_8;
  lowp vec3 tmpvar_9;
  tmpvar_9 = textureCube (_RefractTex, R_4).xyz;
  reflection_3 = tmpvar_9;
  lowp vec4 tmpvar_10;
  ...
  tmpvar_13.xyz = mix (((
    (((reflection_3 * _ReflectionStrength) + (tex_C_Color_5.xyz * _Color.xyz)) + ((Emission_2.xyz * _EmissionColor.xyz) * _Emission))
   + 
    ((_FresnelParams.z + max (0.0, (
      pow (clamp ((1.0 - dot (I_1, N_12)), 0.001, 1.0), _FresnelParams.x)
     * _FresnelParams.y))) * tex_C_Color_5.xyz)
  ) * _ColorScale), xlv_COLOR1.xyz, xlv_COLOR1.www);
  tmpvar_13.w = clamp (clamp (0.0, 1.0, (
    (_Color.w + Emission_2.x)
...
void main ()
{
  ...
  N_13 = xlv_TEXCOORD2;
  finalColor_1 = mix (((
    (((reflection_4 * _ReflectionStrength) + (tex_C_Color_6.xyz * _Color.xyz)) + ((Emission_3.xyz * _EmissionColor.xyz) * _Emission))
   + 
    ((_FresnelParams.z + max (0.0, (
      pow (clamp ((1.0 - dot (I_2, N_13)), 0.001, 1.0), _FresnelParams.x)
     * _FresnelParams.y))) * tex_C_Color_6.xyz)
  ) * _ColorScale), xlv_COLOR1.xyz, xlv_COLOR1.www);
  finalColor_1 = mix (finalColor_1, vec3(dot (finalColor_1, vec3(0.3, 0.59, 0.11))), vec3(_LumiFactor));
  mediump vec4 tmpvar_14;
  tmpvar_14.xyz = finalColor_1;
  tmpvar_14.w = clamp (clamp (0.0, 1.0, (
