...
//
///////////////////////////////////////////
Shader "miHoYo/miHoYo_Scenes/Scenes_Old/Water Shore" {
Properties {
...
void main ()
{
  ...
  tmpvar_7 = xlv_TEXCOORD4.z;
  alpha_2 = tmpvar_7;
  outcolor_1.w = _RefractionColor.w;
  mediump float tmpvar_8;
  tmpvar_8 = max (alpha_2, 0.01);
  lowp vec4 tmpvar_9;
  highp vec2 P_10;
  P_10 = (xlv_TEXCOORD2 - ((texoffset_3 * 5.0) / (tmpvar_8 * tmpvar_8)));
  tmpvar_9 = texture2D (_CausticMap, P_10);
  outcolor_1.xyz = (_RefractionColor.xyz + (tmpvar_9.xyz * _Caustic));
  outcolor_1.xyz = mix (outcolor_1.xyz, xlv_COLOR1.xyz, xlv_COLOR1.www);
  outcolor_1.w = clamp (alpha_2, 0.0, 1.0);
...
{
  highp vec3 tmpvar_1;
  tmpvar_1.xy = xlv_TEXCOORD4.xy;
  mediump vec4 outcolor_2;
  mediump float alpha_3;
  mediump vec2 texoffset_4;
  mediump vec3 accNormal_5;
  lowp vec3 tmpvar_6;
  tmpvar_6 = ((texture2D (_NormalMap, xlv_TEXCOORD0).xyz + texture2D (_NormalMap, xlv_TEXCOORD1).xyz) - 0.6);
  accNormal_5 = tmpvar_6;
  highp vec2 tmpvar_7;
  tmpvar_7 = ((accNormal_5.xy * _Distortion) / xlv_TEXCOORD4.y);
  texoffset_4 = tmpvar_7;
  highp float tmpvar_8;
  tmpvar_8 = xlv_TEXCOORD4.z;
  alpha_3 = tmpvar_8;
  tmpvar_1.z = pow (xlv_TEXCOORD4.z, 4.0);
  lowp vec4 tmpvar_9;
  highp vec2 P_10;
  P_10 = ((xlv_TEXCOORD3.xy / xlv_TEXCOORD3.w) - texoffset_4);
  tmpvar_9 = texture2D (_ReflectionTex, P_10);
  mediump vec4 tmpvar_11;
  tmpvar_11 = (tmpvar_9 * _RefColor);
  highp vec4 tmpvar_12;
  tmpvar_12 = mix (_RefractionColor, tmpvar_11, vec4((xlv_TEXCOORD4.x * tmpvar_1.z)));
  outcolor_2 = tmpvar_12;
  mediump float tmpvar_13;
  tmpvar_13 = max (alpha_3, 0.01);
  lowp vec4 tmpvar_14;
  highp vec2 P_15;
  P_15 = (xlv_TEXCOORD2 - ((texoffset_4 * 5.0) / (tmpvar_13 * tmpvar_13)));
  tmpvar_14 = texture2D (_CausticMap, P_15);
  outcolor_2.xyz = (outcolor_2.xyz + (tmpvar_14.xyz * _Caustic));
  outcolor_2.xyz = mix (outcolor_2.xyz, xlv_COLOR1.xyz, xlv_COLOR1.www);
  outcolor_2.w = clamp (alpha_3, 0.0, 1.0);
...
void main ()
{
  ...
  tmpvar_14 = texture2D (_CausticMap, P_15);
  outcolor_2.xyz = (outcolor_2.xyz + (tmpvar_14.xyz * _Caustic));
  outcolor_2.xyz = (xlv_COLOR1.xyz + (outcolor_2.xyz * clamp (xlv_COLOR1.w, 0.0, 1.0)));
  outcolor_2.w = clamp (alpha_3, 0.0, 1.0);
  gl_FragData[0] = outcolor_2;
}
