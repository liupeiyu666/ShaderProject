...
void main ()
{
  ...
  if (bool(_UseCustomColor)) {
    ColorB_1 = xlv_TEXCOORD1.xyz;
  };
  highp vec3 tmpvar_15;
  tmpvar_15 = mix (ColorA_2, ColorB_1, vec3(clamp ((
    (_LerpBrightness * DissolveChannel_6)
   * MainTexColor_4), 0.0, 1.0)));
  o_9.xyz = (tmpvar_15 * _EmissionScaler);
  if (((DissolveChannel_6 * MainTexAlpha_3) <= (1.0 - xlv_COLOR.w))) {
    o_9.w = 0.0;
  } else {
    o_9.w = 1.0;
  };
  o_9.w = clamp (o_9.w, 0.0, 1.0);
  gl_FragData[0] = o_9;
}
...
void main ()
{
  ...
  tmpvar_21 = mix (ColorA_2, ColorB_1, vec3(clamp ((
    (_LerpBrightness * DissolveChannel_6)
   * MainTexColor_4), 0.0, 1.0)));
  o_12.xyz = (tmpvar_21 * _EmissionScaler);
  if (((DissolveChannel_6 * MainTexAlpha_3) <= (1.0 - xlv_COLOR.w))) {
    o_12.w = 0.0;
  } else {
    o_12.w = 1.0;
  };
  o_12.w = clamp (o_12.w, 0.0, 1.0);
  gl_FragData[0] = o_12;
}
