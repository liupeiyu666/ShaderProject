...
void main ()
{
  mediump vec4 res_1;
  lowp vec4 tmpvar_2;
  tmpvar_2 = texture2D (_Cookie, xlv_TEXCOORD0);
  res_1 = tmpvar_2;
  mediump vec4 color_3;
  color_3 = _MainColor;
  mediump vec4 tmpvar_4;
  if ((_lightProbToggle > 0.5)) {
    tmpvar_4 = _lightProbColor;
  } else {
    tmpvar_4 = vec4(1.0, 1.0, 1.0, 1.0);
  };
  color_3 = (_MainColor * tmpvar_4);
  res_1.w = (((
    (1.0 - res_1.x)
   * _Opaqueness) * _Falloff) * color_3.w);
  mediump float x_5;
  x_5 = (res_1.w - 0.01);
  if ((x_5 < 0.0)) {
    discard;
  };
  res_1.xyz = mix (color_3.xyz, xlv_COLOR1.xyz, xlv_COLOR1.www);
...
void main ()
{
  ...
  res_1.xyz = (xlv_COLOR1.xyz + (color_3.xyz * clamp (xlv_COLOR1.w, 0.0, 1.0)));
  res_1.w = clamp (res_1.w, 0.0, 1.0);
  gl_FragData[0] = res_1;
}
